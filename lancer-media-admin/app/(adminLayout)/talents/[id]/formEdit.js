...
import { updateTalent } from "@/app/services/actions/talents";
...
export default function FormEditTalent({ talent }) {
  const [formState, formAction] = useFormState(updateTalent, initialState);
  const formRef = useRef(null);

  const [editorState, setEditorState] = useState();
  const [contentStateRaw, setContentStateRaw] = useState(talent.talentInfo);
  ...
  useEffect(() => {
    let content = convertFromRaw(JSON.parse(talent.talentInfo));
